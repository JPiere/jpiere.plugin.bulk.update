 ...
 *
 */
public class JPiereBulkUpdateModelFactory implements IModelFactory {

	private static CCache<String,Class<?>> s_classCache = new CCache<String,Class<?>>(null, "PO_Class", 20, false);
	private final static CLogger s_log = CLogger.getCLogger(JPiereBulkUpdateModelFactory.class);

	@Override
	public Class<?> getClass(String tableName)
	{
		if(tableName.startsWith("JP_BulkUpdate"))
		{
			...
			//	Search packages
			StringBuffer name = new StringBuffer("jpiere.plugin.bulk.update.model").append(".M").append(className);
			...
			//	Adempiere Extension
			clazz = getPOclass("jpiere.plugin.bulk.update.model.X_" + tableName, tableName);
			if (clazz != null)
			{
				s_classCache.put(tableName, clazz);
				return clazz;
